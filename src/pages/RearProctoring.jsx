...
const RearProctoring = () => {
  ...
  // STATE BARU: Untuk kontrol kamera
  ...
  // FUNGSI BARU: Untuk pindah kamera
  const toggleCamera = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (count === 1) {
        ...
        if (now - multiPersonStart.current > 2000) setIsSafe(false);
      } 
      else {
        if (!isFirstLockDone) {
          setIsSafe(true);
    ...
    <div className={`min-h-screen flex flex-col transition-colors duration-700 ${isSafe ? 'bg-slate-950' : 'bg-red-950'}`}>
      ...
      {!isLoaded && (
        <div className="fixed inset-0 z-[60] bg-slate-950 flex flex-col items-center justify-center text-white p-10 text-center">
          ...
          <h2 className="text-xl font-black mb-2 italic">Preparing AI...</h2>
        </div>
      )}
      ...
      {isLoaded && !noSleepEnabled && (
        <div className="fixed inset-0 z-50 bg-indigo-600 flex flex-col items-center justify-center p-8 text-center">
          ...
          <h2 className="text-white text-3xl font-black mb-2 uppercase italic">Proctor Cam</h2>
          <button
            onClick={enableNoSleep}
            className="bg-white text-indigo-600 font-black px-12 py-4 rounded-2xl shadow-2xl active:scale-95 transition-all mt-4"
          >
            Mulai Sinkronisasi
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center p-6">
        <div className={`relative aspect-[3/4] w-full max-w-sm mx-auto rounded-[3rem] overflow-hidden border-2 shadow-2xl transition-all duration-700 ${
            isSafe ? 'border-slate-800 shadow-black' : 'border-red-500 scale-[1.02]'
          }`}
        >
          <Webcam
            ref={webcamRef}
            audio={false}
            videoConstraints={{ facingMode: facingMode }} // DINAMIS
          ...
          <button 
            ...
            className="absolute top-6 right-6 z-30 bg-black/50 backdrop-blur-md p-3 rounded-2xl border border-white/10 text-white active:rotate-180 transition-all duration-500"
          >
            <RefreshCw size={20} />
          </button>

          <div className="absolute bottom-6 left-6 bg-black/60 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10">
            <p className="text-white text-[9px] font-black tracking-widest opacity-80 uppercase italic">ID: {userId}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 bg-black/40 grid grid-cols-2 gap-4 border-t border-white/5">
        ...
        <div className="bg-white/5 p-4 rounded-3xl border border-white/5 flex flex-col justify-center">
          <div className="flex items-center gap-2 text-slate-500 mb-1">
            <Zap size={14} />
            <span className="text-[9px] font-black uppercase tracking-widest italic">Mode</span>
          </div>
          <span className="text-sm font-black text-emerald-400 uppercase tracking-tighter italic">
